...
import ProductsBySubCategoryComponent from "@/components/products/ProductsBySubCategoryComponent";
import ProductsComponent from "@/components/products/ProductsComponent";
import CategoryComponent from "@/components/products/category/CategoryComponent";
import CategoryPreviewComponent from "@/components/products/category/CategoryPreviewComponent";
...
export default async function Products({ params, searchParams }: IProducts) {
    ...
    if (!slug) {
        const products = await getProducts(currentPage, undefined, search)
    ...
    if (slug.length === 2 && slug[0] === 'category') {
        ...
        return <CategoryPreviewComponent category={category} />
    }

    if (slug.length === 3 && slug[0] === "category") {
        const products = await getProducts(currentPage, slug[2], undefined)
        return <ProductsBySubCategoryComponent products={products.data} subCategoryName={products.subCategoryName} totalPages={products.totalPages} />
    }
    notFound()
}